...
function Set-InternetProxy($ProxyPerMachine, $TurnProxyOnOff, $proxy, $bypassUrls) {
    ...
    $defaultConnectionSettings = [byte[]]@(@(70, 0, 0, 0, 0, 0, 0, 0, $ProxyEnabled, 0, 0, 0, $proxyBytes.Length, 0, 0, 0) + $proxyBytes + @($bypassBytes.Length, 0, 0, 0) + $bypassBytes + @(1..36 | ForEach-Object { 0 }))

    if ($ProxyPerMachine -eq $True) {
        #ProxySettingsPerMachine
    ...
    Elseif ($ProxyPerMachine -eq $False) {
...
function SetProxySettingsPerUser($Proxy, $ProxyEnable, $defaultConnectionSettings) {
    ...
    Foreach ($UserProfile in $UserProfiles) {
        ...
        If (($ProfileAlreadyLoaded = Test-Path Registry::HKEY_USERS\$($UserProfile.SID)) -eq $false) {
            ...
            Write-Output -ForegroundColor Cyan "Loading hive" $UserProfile.UserHive "for user profile SID:" $UserProfile.SID
        }
        Else {
            Write-Output -ForegroundColor Cyan "Hive already loaded" $UserProfile.UserHive "for user profile SID:" $UserProfile.SID
        }
        ...
        If ($ProfileAlreadyLoaded -eq $false) {
            ...
            Write-Output -ForegroundColor Cyan "Unloading hive" $UserProfile.UserHive "for user profile SID:" $UserProfile.SID
        }
    }
    ...
    Function ClearProxySettingPerUser () {
        ...
        Foreach ($UserProfile in $UserProfiles) {
            ...
            $defaultConnectionSettings = [byte[]]@(@(70, 0, 0, 0, 0, 0, 0, 0, 01, 0, 0, 0, $proxyBytes.Length, 0, 0, 0) + $proxyBytes + @($bypassBytes.Length, 0, 0, 0) + $bypassBytes + @(1..36 | ForEach-Object { 0 }))
